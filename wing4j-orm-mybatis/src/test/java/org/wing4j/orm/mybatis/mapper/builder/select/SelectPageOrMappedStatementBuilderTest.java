...
public class SelectPageOrMappedStatementBuilderTest extends BaseTest {
    @Autowired
    ...
    public void testBuild_upper() throws Exception {
        ...
        config.addInterceptor(new PaginationStage2Interceptor());
        {
            ...
            MappedStatementBuilder builder = new SelectPageOrMappedStatementBuilder(config, DemoCrudMapper.class);
            builder.setKeywordMode(WordMode.lowerCase);
            builder.setSqlMode(WordMode.lowerCase);
            builder.setSchemaMode(TableNameMode.auto);
            builder.setPrefixMode(TableNameMode.auto);
            ...
            demoEntity.setCol3(1);
            {
                ...
                List<DemoEntity> list = sqlSession.selectList(Constants.SELECT_PAGE_OR, pagination);
                pagination.setRecords(list);
                Assert.assertEquals(3, pagination.getTotal());
                Assert.assertEquals(1, pagination.getCurPageNo());
                Assert.assertEquals(2, pagination.getPageSize());
                Assert.assertEquals(2, list.size());
            }
            {
                Pagination<DemoEntity> pagination = new Pagination<DemoEntity>(2, 2, demoEntity);
                List<DemoEntity> list = sqlSession.selectList(Constants.SELECT_PAGE_OR, pagination);
                pagination.setRecords(list);
                Assert.assertEquals(3, pagination.getTotal());
                Assert.assertEquals(2, pagination.getCurPageNo());
                Assert.assertEquals(2, pagination.getPageSize());
